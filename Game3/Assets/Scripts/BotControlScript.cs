...
public class BotControlScript : MonoBehaviour
{
	...
	void Start ()
	{
		...
		enemy = GameObject.Find("final").transform;	
	...
	void Update()
	{
		...
		{
			if(robot.transform.position.y - rbCurrentPos.y < 1.8)
			{
			robot.transform.Translate (0, 5*Time.deltaTime, 0);	
			//Debug.Log(robot.transform.position.y);
		}
		}
		else{
			rbCurrentPos = robot.transform.position;
		}
	}
	void FixedUpdate ()
	{
		...
		else if(currentBaseState.nameHash == jumpState)
		{
			...
			//
			anim.transform.position = new Vector3 (anim.transform.position.x, anim.transform.position.y + Time.deltaTime,anim.transform.position.z);
